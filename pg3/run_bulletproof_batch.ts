...
import { UnifiedDiscoveryService, DiscoveryMode, DiscoveryResult } from './src/core/discovery/unified_discovery_service';
import { BrowserFactory } from './src/core/browser/factory_v2';
import { Logger } from './src/utils/logger';
import { CompanyInput } from './src/core/company_types';
import { AntigravityClient } from './src/observability/antigravity_client';
import { EnvValidator } from './src/utils/env_validator';
...
async function executeRun(runId: number, mode: DiscoveryMode, companies: CompanyInput[]) {
    ...
    const tasks = pending.map(company => limit(async () => {
        try {
            ...
            const enriched = { ...company, ...enrichCompanyWithResult(company, res) };
