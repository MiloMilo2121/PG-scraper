...
async function runBenchmark() {
    console.log("🚀 OMEGA V7.0 - SPRINT 1 QUICK CHECK (100 COMPANIES) 🚀\n");
    ...
    const targetCompanies = records.slice(0, 5); // Testing only 5 for speed!
    ...
    for (let i = 0; i < companiesToTest.length; i++) {
        ...
        try {
            console.log(`   [Action] Launching OMEGA V7 FULL Pipeline (FAST_RUN1)...`);
            const result = await service.discover(c, DiscoveryMode.FAST_RUN1);

            if (result.status === 'FOUND_VALID') {
                console.log(`   ✅ TROVATO: ${result.url} (Conf: ${result.confidence.toFixed(2)})`);
                successCount++;
